...
export class RequestsComponent implements OnInit {
  ...
  loadData() {
    this.db.doc$('requests/--index--').subscribe(
      (data: any) => {
        if (!data && !this.helper.onLine) {
          this.offline = true;
          return;
        }
        if (!data) return;
        ...
        if (
          this.auth.user.newAdminRequest &&
          !(data.requests as any[]).filter(
            r =>
              !(JSON.parse(
                localStorage.getItem(adminRequestsKey)
              ) as string[]).includes(r.id)
          ).length
        ) {
        ...
        this.data.filterPredicate = (
          ...
          return (
            matchFilter.some(match => match == true) || !matchFilter.length
          );
        };
        this.prepareView();
      },
